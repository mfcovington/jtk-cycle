...
hlm <- function(z) {
        ...
        median(zz)/2     ## median of the Walsh averages
}
...
jtkx <- function(z, conf=0.9) {
  ...
  for(i in 1:length(pers)) {
    ...
    for(lagi in lagis[[i]]) {  
      ...
      signcos <- sc[,lagi]
      tmp <- s*w*signcos						##
			wt <- wilcox.test(tmp,conf.int=TRUE,conf.level=conf,exact=FALSE)
  ...
  JTK.TAU <<- abs(S)/JTK.MAX                      # note: all optimal abs(S) are the same
}
